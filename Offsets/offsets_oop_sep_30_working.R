...
plot_outs <- function(...){
  ...
  for (s in 1:plot_params[3]){
    plot(dots[[1]][ , s], type = 'l', ylim = c(ymin, ymax))
    if (plot_num > 1){
      for (t in 2:plot_num){
        lines(dots[[t]][ , s], col = 'red', ylim = c(ymin, ymax))
      }
    } 
  }
}
...
net_impact_sums <- sum_regions(params$time_steps, parcels, net_impacts)
...
plot_outs(true_gains_sums, developed_sums, net_impact_sums, plot_params)

plot_outs(counterfactual_sums, region_sums, plot_params)
