...
initialise_ecology <- function(global_params, parcels){
  ...
  ecology_initial = matrix(1,global_params$ecology_size,global_params$ecology_size)
  ...
  ecology_initial = ecology_initial + global_params$initial_eco_noise*matrix(runif(global_params$ecology_size*global_params$ecology_size),global_params$ecology_size,global_params$ecology_size)
...
ecology_change_function <- function(pos_y, min_eco_val, max_eco_val, rate, t){
  t_sh = -1/rate * log( ((pos_y - min_eco_val)/(max_eco_val - pos_y)))
  ecology_change_function = min_eco_val + (max_eco_val - min_eco_val)/(1 + exp(-rate*( t - t_sh)))
}

ecology_change_function_vectorised <- function(parcel_vals, min_eco_val, max_eco_val, curve_rate, time_remaining){
  time_array = matrix(rep(time_remaining, length(parcel_vals)), ncol = length(parcel_vals))
  t_sh = -1/curve_rate * log( ((parcel_vals - min_eco_val)/(max_eco_val - parcel_vals)))
  t_sh_array = matrix(rep(t_sh, length(time_remaining)), ncol = length(time_remaining))
  t_sh_array <- t(t_sh_array)
  ecology_change_function = min_eco_val + (max_eco_val - min_eco_val)/(1 + exp(-curve_rate*( time_array - t_sh_array)))
  return(ecology_change_function)
}
...
build_counterfactuals <- function(region_num, regions, global_params, decline_rates, land_parcels, ecology_initial, time_vec){
  elements = array(0, c(global_params$ecology_size, global_params$ecology_size, global_params$time_steps))
  for (region_ind in 1:region_num){ 
    ...
    for (parcel_ind in 1:current_parcel_num){
      ...
      predicted_traj = ecology_change_function_vectorised(ecology_initial[current_parcel], global_params$min_eco_val, global_params$max_eco_val, current_dec_rate, time_vec)
      for (pix_ind in 1:length(current_parcel)){
        loc = ind2sub(global_params$ecology_size, current_parcel[pix_ind])
        elements[loc[1], loc[2], 1:global_params$time_steps] = predicted_traj[, pix_ind]        
      }
    }
  }
  return(elements)
...
build_counterfactuals_blur <- function(region_num, regions, global_params, decline_rates, land_parcels, ecology_initial, time_vec){
  elements = array(0, c(global_params$ecology_size, global_params$ecology_size, global_params$time_steps))
  for (yr in 1:global_params$time_steps){
    ...
    for (region_ind in 1:region_num){ 
      ...
      for (parcel_ind in 1:current_parcel_num){
        ...
        current_parcel = land_parcels[[current_parcel_ind]]
        current_dec_rate = current_decline_rates[parcel_ind]      
        predicted_traj = ecology_change_function_vectorised(ecology_initial[current_parcel], min_eco_val, max_eco_val, current_dec_rate, time_vec)
        for (pix_ind in 1:length(current_parcel)){
          loc = ind2sub(global_params$ecology_size, current_parcel[pix_ind])
...
initialise_counterfactuals_vectorised <- function(global_params, region_params, parcels){
  counterfactuals = list() 
  ecology_initial <- initialise_ecology(global_params, parcels)
  
  region_num = parcels$region_num
  regions = parcels$regions
  land_parcels = parcels$land_parcels
  time_vec = 1:global_params$time_steps 
  
  decline_rates = build_decline_rates(parcels$region_num, parcels$regions, region_params)
  elements = build_counterfactuals(region_num, regions, global_params, decline_rates, land_parcels, ecology_initial, time_vec)
  final_ecology = elements[, , global_params$time_steps]
...
build_decline_rates <- function(region_num, regions, region_params){
...
initialise_outputs <- function(counterfactuals, parcels){
...
update_index <- function(index_object, current_parcel_indexes, region_ind, max_region_dev_num){
...
write_development_trajectory <- function(trajectories, development_object, ecology_size, time_steps, yr){
  ...
  for (pix_ind in current_parcel){
    loc = ind2sub(ecology_size, pix_ind)
    trajectories[loc[1], loc[2], (yr + 1):time_steps] = 0
  }   
  return(trajectories)
}


find_offset_pool <- function(index_object, region_ind, global_params, region_params, parcels, current_ecology, time_remaining, final_ecology){
...
predict_final_parcel_vals <- function(parcels, parcel_pool, current_ecology, min_eco_val, max_eco_val, curve_rate, time_remaining){
  
  land_parcels = parcels$land_parcels
  predicted_parcel_trajs = predict_parcel_trajectories(parcels, parcel_pool, min_eco_val, max_eco_val, current_ecology, curve_rate, time_remaining)
  predicted_parcel_vals = predicted_parcel_trajs[, length(time_remaining)]                                  
  return(predicted_parcel_vals)  
  
}

predict_parcel_trajectories <- function(parcels, parcel_pool, min_eco_val, max_eco_val, current_ecology, curve_rate, time_remaining){
  ...
  for (parcel_ind in 1:length(parcel_pool)){
    ...
    current_parcel = land_parcels[[current_parcel_ind]]
    predicted_pixel_trajs = ecology_change_function_vectorised(current_ecology[current_parcel], min_eco_val, max_eco_val, curve_rate, time_remaining)
...
write_offset_trajectory_vectorised <- function(trajectories, offset_object, current_ecology, min_eco_val, max_eco_val, ecology_size, time_steps, offset_rate, time_remaining, yr){
  ...
  for (parcel_ind in 1:parcel_num){
    ...
    predicted_traj = ecology_change_function_vectorised(current_ecology[current_parcel], min_eco_val, max_eco_val, offset_rate, time_remaining)
    for (pix_ind in 1:length(current_parcel)){
      loc = ind2sub(ecology_size, current_parcel[pix_ind])
      trajectories[loc[1], loc[2], yr:time_steps] = predicted_traj[, pix_ind]        
    }
  }
  return(trajectories)
}
...
calc_diffs <- function(parcel_list, trajectories, ecology_size, time_steps, parcels, counterfactuals){
  graphics.off()
  outputs = list()
  ...
  for (yr in 1:time_steps){
    ...
    for (parcel_ind in (parcel_list[[yr]])){
      ...
      for (pix_ind in current_parcel){
        ...
        true_change[loc[1], loc[2], yr:time_steps] = current_trajectory - counterfactuals$elements[loc[1], loc[2], yr:time_steps]       
        traj_rel_to_initial[loc[1], loc[2], yr:time_steps] = current_trajectory - initial_val
        initial_rel_to_counter[loc[1], loc[2], yr:time_steps] = initial_val - counterfactuals$elements[loc[1], loc[2], yr:time_steps]     
      }
    }
  }
...
view_individual_offsets <- function(parcel_set, parcels, time_steps, trajectories, counterfactuals){
  ...
  dev_counterfactual = find_parcel_trajectory(parcels, developed_parcel, time_steps, counterfactuals$elements)
  
  offset_trajectories = find_parcel_trajectories(parcels, offset_parcels, time_steps, trajectories)
  
  offset_counterfactuals = find_parcel_trajectories(parcels, offset_parcels, time_steps, counterfactuals$elements)
...
calc_trajectories <- function(global_params, region_params, outputs, parcels, index_object, counterfactuals, perform_offsets){
  dev_count = 1;
      ...
      for (region_ind in 1:parcels$region_num){
        ...
        if (current_develop_num > 0){
          for (dev_index in 1:current_develop_num){    #loop over the number of possible developments           
            ...
            development_object <- update_development_object(region_params[[region_ind]]$development_value_type, index_object$ind_available[[region_ind]], parcels, yr, current_ecology, counterfactuals$final_ecology)
            outputs$trajectories <- write_development_trajectory(outputs$trajectories, development_object, global_params$ecology_size, global_params$time_steps, yr)     
            index_object <- update_index(index_object, development_object$parcel_index, region_ind, region_params[[region_ind]]$max_region_dev_num)      
            if (index_object$break_flag == TRUE){
              break
            } 
            
            if (perform_offsets == TRUE){
              offset_pool <- find_offset_pool(index_object, region_ind, global_params, region_params, parcels, current_ecology, time_remaining, counterfactuals$final_ecology)
              ...
              if (region_params[[region_ind]]$offset_policy == 'improve'){
                outputs$trajectories = write_offset_trajectory_vectorised(outputs$trajectories, offset_object, current_ecology, global_params$min_eco_val, global_params$max_eco_val, global_params$ecology_size, global_params$time_steps, region_params[[region_ind]]$offset_rate, time_remaining, yr)
              } 
              
              index_object = update_index(index_object, offset_object$current_parcel_indexes, region_ind, region_params[[region_ind]]$max_region_dev_num)           
              if (index_object$break_flag == TRUE){
                break
              }               
