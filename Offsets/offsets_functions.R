...
Blur_2D <- function(A, sig_x, sig_y){
  ...
  g = gauss(mesh$x, mesh$y, sig_x, sig_y)
  g = g / sum(g)
  convolve = fftshift( fft( fftshift(g), inverse = TRUE )) * fftshift( fft( fftshift(A), inverse = TRUE ) )
...
Blur_2D <- function(A, sig_x, sig_y){
  ...
  g = gauss(mesh$x, mesh$y, sig_x, sig_y)
  gauss = g / sum(g)
  convolve = fft(g, inverse = TRUE) * fft(A, inverse = TRUE)
  A = fft(convolve)*(M*N)
  A = Re(A)
  return(A)
}
