...
select_parcel_to_offset <- function(params, parcels, developed_object, index_object, current_parcel_vals, predicted_parcel_vals, predict_flag){
  offset_object = list()
...
write_developed_trajectory <- function(trajectories, developed_object, current_ecology, params, yr){
...
write_offset_trajectory <- function(outputs, current_parcel, current_ecology, params, time_remaining, yr){
  ...
  for (pix_ind in current_parcel){
    ...
    #outputs$true_gains[loc[1], loc[2], yr:params$time_steps] = current_trajectory - counterfactual             
  }
  return(outputs)
...
for (yr in 1:params$time_steps){      #main time loop
  ...
  if (develop_num > 0){
    ...
    for (dev_index in 1:develop_num){    #loop over the number of possible developments           
      ...
      offset_object <- select_parcel_to_offset(params, parcels, developed_object, index_object, current_parcel_vals, predicted_parcel_vals, predict_flag)      
      ...
      outputs$avoided_loss_static <- write_current_value(outputs$avoided_loss_static, offset_object$current_parcel, current_ecology, params, yr) 
