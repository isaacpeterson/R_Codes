initialise_parameters <- function(){
  ...
  params$offset_rate = 0.02 #rate of improvement in land parcel after parcel is offset
  params$eco_noise = 20
...
initialise_offset_object <- function(params){
  object = list()
  object$list = vector('list', params$time_steps)
  return(object)
}
...
initialise_outputs <- function(params, counterfactuals, parcels){
  ...
  outputs$avoided_losses = outputs$offset_trajectories
  outputs$avoided_loss_static = outputs$offset_trajectories
  outputs$true_losses = outputs$offset_trajectories
...
select_parcel_to_develop <- function(developed_object, ind_available, current_parcel_vals, parcels){
  ...
  developed_object$list[[yr]] = c(developed_object$list[[yr]], parcel_index)
...
select_parcel_to_offset <- function(offset_object, params, parcels, developed_object, index_object, current_parcel_vals, predicted_parcel_vals, predict_flag){
  ...
  if (offset_type == 'match') {    
    ...
    while (any(val_to_match > parcel_vals_to_use)){
      err = abs(val_to_match - parcel_vals_to_use)
      err_ind = which(err == min(err))
      if (length(err_ind) > 1){   #if more than one parcel is of equal value chose at random
       err_ind = sample(err_ind)
      }
      parcel_val = parcel_vals_to_use[err_ind]       
      parcel_indexes = c(parcel_indexes, offset_ind_to_use[err_ind])     
      parcel_vals = c(parcel_vals, parcel_val)
      val_to_match = val_to_match - parcel_val
      offset_ind_to_use = offset_ind_to_use[-err_ind]
      parcel_vals_to_use = current_parcel_vals[offset_ind_to_use]
  ...
  offset_object$list[[yr]] = c(offset_object$list[[yr]], parcel_indexes)
...
write_developed_trajectory <- function(outputs, developed_object, current_ecology, params, yr){
  ...
  for (pix_ind in current_parcel){
    ...
    outputs$developed_losses[loc[1], loc[2], yr:params$time_steps] = current_trajectory 
  }
    return(trajectories)
}

write_offset_trajectory <- function(outputs, offset_object, current_ecology, params, time_remaining, yr){
  ...
  for (parcel_ind in 1:parcel_num){
    ...
    for (pix_ind in current_parcel){
      ...
      outputs$offset_trajectories[loc[1], loc[2], yr:params$time_steps] = current_trajectory       
    }
  }
  return(outputs)
}
...
for (yr in 1:params$time_steps){      #main time loop
  ...
  if (develop_num > 0){
    ...
    for (dev_index in 1:develop_num){    #loop over the number of possible developments           
      ...
      outputs$trajectories <- write_developed_trajectory(outputs$trajectories, developed_object, current_ecology, params, yr)      
      ...
      outputs = write_offset_trajectory(outputs, offset_object, current_ecology, params, time_remaining, yr)         
...
plot_type = 'net region impacts'  # 'developments vs offsets', 'measured vs counterfactual', or 'true region gains' 
