initialise_parameters <- function(){
  ...
  params$parcel_num_y = 10 #numnber of parcels in x
  ...
  params$max_decline_rate = 0.0002 #max regional decline rate parameter used in logisitic equation
...
find_current_develop_num <- function(params, region_ind){
  current_develop_num = params$region_dev_num[region_ind]
  return(current_develop_num)
}
  ...
  for (yr in 1:params$time_steps){      #main time loop    
    ...
    if (yr%%params$develop_every == 0 && yr < params$time_steps){
      ...
      for (region_ind in 1:parcels$region_num){
        ...
        current_develop_num <- find_current_develop_num(params, region_ind)
        outputs$adjusted_counterfactuals <- update_counterfactuals(outputs$adjusted_counterfactuals, index_object$ind_available[[region_ind]], current_develop_num, parcels$land_parcels, yr)
...
update_counterfactuals <- function(adjusted_counterfactuals, current_parcel_set, current_develop_num, land_parcels, yr){
...
outputs_offsets_off <- calc_trajectories(params, outputs, parcels, index_object, counterfactuals, perform_offsets = FALSE)
outputs_offsets_on <- calc_trajectories(params, outputs, parcels, index_object, counterfactuals, perform_offsets = TRUE)

parcels_object_offsets_off <- collate_parcel_sets(outputs_offsets_off$parcel_sets, params)
parcels_object_offsets_on <- collate_parcel_sets(outputs_offsets_on$parcel_sets, params)

offsets_off_trajectory_sums <- find_sums(params$time_steps, parcels, outputs_offsets_off$trajectories)
offsets_on_trajectory_sums <- find_sums(params$time_steps, parcels, outputs_offsets_on$trajectories)
counterfactual_sums <- find_sums(params$time_steps, parcels, counterfactuals$elements)
adjusted_counterfactual_sums_offsets_on <- find_sums(params$time_steps, parcels, outputs_offsets_on$adjusted_counterfactuals)
adjusted_counterfactual_sums_offsets_off <- find_sums(params$time_steps, parcels, outputs_offsets_off$adjusted_counterfactuals)

plot_params = c(params$region_num_x, params$region_num_y, parcels$region_num)
plot_outs(offsets_on_trajectory_sums$region_sums, offsets_off_trajectory_sums$region_sums, counterfactual_sums$region_sums, plot_params)

plot_outs(adjusted_counterfactual_sums_offsets_off$region_sums, counterfactual_sums$region_sums, plot_params)

#offset_outs <- calc_diffs(parcels_object$offset_parcels, outputs$trajectories, params, parcels, counterfactuals)
#development_outs <- calc_diffs(parcels_object$developed_parcels, outputs$trajectories, params, parcels, counterfactuals)
...
# counterfactual_sums <- find_sums(params$time_steps, parcels, counterfactuals$elements)
# development_absolute_sums <- find_sums(params$time_steps, parcels, offset_outs$offset_trajectories)
# 
# true_offset_gains <- find_sums(params$time_steps, parcels, offset_outs$true_change)
# true_development_losses <- find_sums(params$time_steps, parcels, development_outs$true_change)
# 
# plot_params = c(params$region_num_x, params$region_num_y, parcels$region_num)
# #plot_outs(true_offset_gains$region_sums, true_development_losses$region_sums, plot_params)
# 
# plot_outs(trajectory_sums$region_sums, counterfactual_sums$region_sums, true_development_losses$region_sums, plot_params)
...
#   #      current_offset_pool = intersect(regions[[s]], ind_available)                                        
#   #      break
#   #    }
#   #  }
#   #} else if (parcel_selection_type == 'national'){ 
#   #  current_offset_pool = ind_available
#   #}
#   
#   if (length(current_offset_pool) < 1 ){   
#     offset_object$break_flag = TRUE
#     return(offset_object)
#   } else {offset_object$break_flag = FALSE}
#   
#   return(current_offset_pool)
# }
