...
select_parcel_to_develop <- function(development_object, ind_available, current_parcel_vals, parcels){
  ...
  development_object$parcel_val = parcel_val
...
select_offset_parcel <- function(offset_ind_to_use, offset_object, params, parcels, development_object, index_object, current_parcel_vals, predicted_parcel_vals, predict_flag){      
  ...
  parcel_vals = vector()
  val_to_match = params$offset_multiplier*development_parcel_val
...
write_development_trajectory <- function(trajectories, counterfactuals, development_object, current_ecology, params, yr){
  ...
  for (pix_ind in current_parcel){
    loc = ind2sub(params$ecology_size, pix_ind)
    trajectories[loc[1], loc[2], yr:params$time_steps] = 0
  }   
  return(trajectories)
}

write_offset_trajectory <- function(trajectories, counterfactuals, offset_object, current_ecology, params, time_remaining, yr){
...
sum_regions <- function(parcel_sums, parcels){
...
find_sums <- function(time_steps, parcels, trajectories){
  ...
  object$region_sums = sum_regions(object$parcel_sums, parcels)
  #object$net_sums = sumCols(object$region_sums)
  return(object)
}
...
determine_trajectories <- function(params, parcels, index_object, offset_object, development_object, counterfactuals, outputs){
    ...
    for (region_ind in 1:parcels$region_num){
      ...
      if (develop_num > 0){
        ...
        for (dev_index in 1:develop_num){    #loop over the number of possible developments           
          ...
          development_object <- select_parcel_to_develop(development_object, index_object$ind_available[[region_ind]], current_parcel_vals, parcels)      
          outputs <- write_development_trajectory(outputs, counterfactuals, development_object, current_ecology, params, yr)               
          ...
          offset_object <- select_offset_parcel(offset_ind_to_use, offset_object, params, parcels, development_object, index_object, current_parcel_vals, predicted_parcel_vals, predict_flag)      
...
development_object <- initialise_offset_object(params)
counterfactuals <- initialise_counterfactuals(params, parcels, ecology_initial)

#outputs <- determine_trajectories(params, parcels, index_object, offset_object, development_object, counterfactuals, outputs)
    ...
    for (region_ind in 1:parcels$region_num){
      ...
      if (develop_num > 0){
        ...
        for (dev_index in 1:develop_num){    #loop over the number of possible developments           
          ...
          outputs$offset_object <- select_offset_parcel(offset_ind_to_use, outputs$offset_object, params, parcels, development_object, index_object, current_parcel_vals, predicted_parcel_vals, predict_flag)      
          
          if (outputs$offset_object$break_flag == TRUE){
            break
          } 
          
          outputs$trajectories = write_offset_trajectory(outputs$trajectories, counterfactuals, outputs$offset_object, current_ecology, params, time_remaining, yr)         
...
offset_outs = determine_offs(outputs$offset_object, outputs$trajectories, params, parcels)
development_outs <- determine_devs(development_object, params, parcels)
...
static_losses <- find_sums(params$time_steps, parcels, development_outs$static_losses)
true_offset_gains <- find_sums(params$time_steps, parcels, offset_outs$true_offset_gains)
true_losses <- find_sums(params$time_steps, parcels, development_outs$development_losses)
offset_gains_rel_to_current <- find_sums(params$time_steps, parcels, offset_outs$offset_gain_rel_to_current)
avoided_degredation <- find_sums(params$time_steps, parcels, offset_outs$avoided_degredation)
 
plot_params = c(params$region_num_x, params$region_num_y, parcels$region_num)

plot_outs(net_impacts$region_sums, true_offset_gains$region_sums, true_losses$region_sums, plot_params)

plot_outs(net_impacts$region_sums, true_offset_gains$region_sums, -true_losses$region_sums, offset_absolute_sums$region_sums, plot_params)

# plot_outs(true_offset_gains$region_sums, avoided_degredation$region_sums, offset_gains_rel_to_current, plot_params)
# 
