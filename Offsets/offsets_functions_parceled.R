...
initialise_index_object <- function(parcels, global_params){
  ...
  index_object$development_list = vector()
  index_object$offset_list = vector()
...
build_counterfactuals_by_parcel <- function(global_params, decline_rates_array, land_parcels, initial_ecology){
  ...
  for (yr in 1:global_params$time_steps){
    for (parcel_ind in 1:parcel_num){
      ...
      current_dec_rate = decline_rates_array[parcel_ind]      
...
initialise_counterfactuals_vectorised <- function(global_params, region_params, land_parcels, initial_ecology, decline_rates_array){
  time_steps = global_params$time_steps 
  ecology_size = global_params$ecology_size
  counterfactuals = build_counterfactuals_by_parcel(global_params, decline_rates_array, land_parcels, initial_ecology)
  return(counterfactuals)
}
...
build_decline_rates <- function(parcels, region_params){
  ...
  #decline_rates$region_list = dec_rates
  decline_rates = decline_rates_array
  return(decline_rates)
}


initialise_outputs <- function(global_params){
  outputs = list()
...
update_development_object <- function(region_params, global_params, region_ind, ind_available, current_ecology, decline_rates, land_parcels, yr){
  ...
  if (dev_value_type == 'current condition'){
    parcel_vals_used = current_parcel_val
  } else if (dev_value_type == 'future condition'){
    parcel_vals_used = predict_parcel_vals(predict_type = 'protect', current_ecology, parcel_inds, land_parcels, decline_rates, restoration_rate, 
...
predict_parcel_vals <- function(predict_type, current_ecology, parcel_inds, land_parcels, decline_rates, restoration_rate, min_eco_val, max_eco_val, time_step){
  ...
  for (parcel_ind in 1:parcel_num){
    ...
    parcel_ecology = current_ecology[current_parcel]
    if (predict_type == 'protect'){
      decline_rate = current_decline_rates[current_parcel_ind]
      predicted_parcel = sapply(parcel_ecology, eco_change, min_eco_val, max_eco_val, decline_rate, time_step)
    } else if(predict_type == 'restore'){
      decline_rate = restoration_rate
      if (decline_rate == 1){
        predicted_parcel = parcel_ecology
      } else {predicted_parcel = sapply(parcel_ecology, eco_change, min_eco_val, max_eco_val, decline_rate, time_step)}
    }
...
find_offset_pool <- function(index_object, region_ind, decline_rates, min_eco_val, max_eco_val, time_steps, region_params, parcels, current_ecology, time_remaining){
  
  offset_pool = list()
  land_parcels = parcels$land_parcels
  offset_selection_type = region_params[[region_ind]]$offset_selection_type
  ...
  parcel_num = length(parcels$regions[[region_ind]])
  
  if ( (offset_selection_type == 'avoided loss') || (offset_selection_type == 'restoration from counterfactual')){
    counterfactual_vals_pool = predict_parcel_vals(predict_type = 'protect', current_ecology, parcel_inds = current_offset_pool, land_parcels, decline_rates, region_params[[region_ind]]$restoration_rate,
                                                   min_eco_val, max_eco_val, time_step = length(time_remaining))
    if (global_params$include_avoided_clearing == TRUE){
      survival_prob = (1 - length(index_object$development_list)/parcel_num)
      counterfactual_vals_pool = counterfactual_vals_pool*survival_prob
    }
  }
  
  if ( (offset_selection_type == 'restoration gains') || (offset_selection_type == 'restoration from counterfactual') ){
    offset_vals_pool = predict_parcel_vals(predict_type = 'restore', current_ecology, parcel_inds = current_offset_pool, land_parcels, decline_rates, region_params[[region_ind]]$restoration_rate,
                                           min_eco_val, max_eco_val, time_step = length(time_remaining))
  }
   
  if (offset_selection_type == 'current condition'){parcel_vals_pool = current_parcel_sums} 
  else if (offset_selection_type == 'restoration gains'){parcel_vals_pool = offset_vals_pool - current_parcel_sums} 
  else if (offset_selection_type == 'avoided loss'){parcel_vals_pool = current_parcel_sums - counterfactual_vals_pool} 
  else if (offset_selection_type == 'restoration from counterfactual'){parcel_vals_pool = offset_vals_pool - counterfactual_vals_pool}
...
record_parcel_info <- function(out_object, parcel_indexes, land_parcels, current_ecology,  parcel_vals_used){
  ...
  current_parcels = list()
  parcel_sums = array(0, parcel_num)
  parcel_ecologies = vector('list', parcel_num)
  
  for (parcel_ind in 1:parcel_num){
    
    current_parcel = land_parcels[[parcel_indexes[parcel_ind]]]
    current_parcels[[parcel_ind]] = current_parcel
    parcel_ecology = current_ecology[current_parcel]
    dim(parcel_ecology) = dim(current_parcel)
    parcel_ecologies[[parcel_ind]] = parcel_ecology
    parcel_sums[parcel_ind] = sum(current_ecology[current_parcel])
    
  }  
  
  out_object$parcel_ecologies = parcel_ecologies
...
write_null_offset_object <- function(){
  ...
  offset_object$parcel_sums = list()
...
write_developments_offsets <- function(index_object, development_object, offset_object, region_ind, yr){
  index_object$development_list = c(index_object$development_list, development_object$parcel_indexes)
  index_object$offset_list = c(index_object$offset_list, offset_object$parcel_indexes)
...
update_ind_available <- function(update_type, index_object, parcel_indexes, region_ind, current_develop_num){
  
  if (update_type == 'development'){
    index_object$development_list = c(index_object$development_list, parcel_indexes)
  } else if (update_type == 'offset'){
    index_object$offset_list = c(index_object$offset_list, parcel_indexes)
  }
...
update_decline_rates <- function(decline_rates, parcel_indexes, decline_update_type, region_params, region_ind){
  
  if (decline_update_type == 'development'){
    update_val = 0
  } else if (decline_update_type == 'offset'){
    if (region_params[[region_ind]]$offset_policy_action_type == 'maintain'){
      update_val = 1
    } else if (region_params[[region_ind]]$offset_policy_action_type == 'restore'){
      update_val = region_params[[region_ind]]$restoration_rate
    } else if (region_params[[region_ind]]$offset_policy_action_type == 'protect'){
      return(decline_rates)
    }
  }
  ...
  for (parcel_ind in 1:length(parcel_indexes)){
    ...
    decline_rates[current_parcel_ind] = update_val
  }
  return(decline_rates)
}


find_current_dev_nums <- function(global_params, region_params, yr){
...
calc_trajectories_by_parcel <- function(global_params, region_params, current_ecology, decline_rates, parcels, index_object, perform_offsets, record_parcel_sets){
  
  trajectories = array(0, c(global_params$ecology_size, global_params$ecology_size, global_params$time_steps))
  
  for (yr in 1:global_params$time_steps){      #main time loop    
    time_remaining = 0:(global_params$time_steps - yr)     
    current_dev_nums <- find_current_dev_nums(global_params, region_params, yr) 
    ...
    for (region_ind in 1:parcels$region_num){       
      ...
      if (current_develop_num > 0){
        ...
        for (dev_index in 1:current_develop_num){    #loop over the number of possible developments           
          ...
          index_object <- update_ind_available(update_type = 'development', index_object, development_object$parcel_indexes, region_ind, current_develop_num)      
          decline_rates <- update_decline_rates(decline_rates, development_object$parcel_indexes, decline_update_type = 'development', region_params, region_ind)
          
          if (index_object$break_flag == TRUE){
            break
          }                      
          if (perform_offsets == TRUE){
            offset_pool <- find_offset_pool(index_object, region_ind, decline_rates, global_params$min_eco_val, global_params$max_eco_val, global_params$time_steps, region_params, parcels, current_ecology, time_remaining)
            ...
            index_object = update_ind_available(update_type = 'offset', index_object, offset_object$parcel_indexes, region_ind, current_develop_num)              
            decline_rates <- update_decline_rates(decline_rates, offset_object$parcel_indexes, decline_update_type = 'offset', region_params, region_ind)
  ...
  if (record_parcel_sets == TRUE){
    outputs = list()
    outputs$offset_list = index_object$offset_list
    outputs$development_list = index_object$development_list
...
update_ecology_by_parcel <- function(current_ecology, land_parcels, decline_rates_array, min_eco_val, max_eco_val, time_step){
  parcel_num = length(land_parcels)
  
  for (parcel_ind in 1:parcel_num){ 
    
    current_parcel = land_parcels[[parcel_ind]]
    decline_rate = decline_rates_array[parcel_ind]
...
initialise_parcel_set_list <- function(parcel_set_num){
  ...
  object$parcel_set_trajectories = vector()
  object$parcel_set_counters = vector()
...
# land_parcels = parcels$land_parcels
# parcel_sets = outputs$parcel_sets
# trajectories = outputs$trajectories
# time_steps = global_params$time_steps
# parcel_sets_object = parcel_sets_object$developments
# current_parcel_set_object = parcel_set$development_object

assess_parcel_sets <- function(parcel_sets, land_parcels, trajectories, time_steps, decline_rates_initial){
  ...
  for (parcel_set_ind in 1:parcel_set_num){
    ...
    yr = parcel_set$yr
    parcel_sets_object$developments = assess_parcel_set_new(global_params, region_params, parcel_sets_object$developments, parcel_set$development_object, 
                                                            parcel_set_ind, land_parcels, trajectories, yr, time_steps, decline_rates_initial)
    
    if (length(parcel_set$offset_object$parcel_indexes) > 0){
      parcel_sets_object$offsets = assess_parcel_set_new(global_params, region_params, parcel_sets_object$offsets, parcel_set$offset_object, 
                                                         parcel_set_ind, land_parcels, trajectories, yr, time_steps, decline_rates_initial)
...
assess_parcel_set_new <- function(global_params, region_params, parcel_sets_object, current_parcel_set_object, parcel_set_index, land_parcels, 
                                  trajectories, yr, time_steps, decline_rates_initial){
  
  parcel_inds = current_parcel_set_object$parcel_indexes
  initial_parcel_ecologies = current_parcel_set_object$parcel_ecologies
  ...
  initial_rel_counter = array(0, c(time_steps, parcel_num))
  traj_rel_initial = array(0, c(time_steps, parcel_num))

  parcel_counter_sum = array(0, c(time_steps, parcel_num))
  parcel_trajectory_sum = array(0, c(time_steps, parcel_num))
  initial_sum = array(0, parcel_num)
  ...
  for (parcel_count_ind in 1:parcel_num){
    ...
    parcel_ecology = initial_parcel_ecologies[[parcel_count_ind]]
    parcel_counterfactual = find_parcel_counterfactual(current_parcel_trajectory, parcel_ecology, decline_rates_initial[current_parcel_ind], global_params, yr)
    
    traj_rel_initial[, parcel_count_ind] = calc_rel_intial(trajectory_type = 'trajectory', current_parcel, time_steps, current_parcel_trajectory, yr, parcel_ecology)
    initial_rel_counter[, parcel_count_ind] = calc_rel_intial(trajectory_type = 'counterfactual', current_parcel, time_steps, parcel_counterfactual, yr, parcel_ecology)
    
    parcel_trajectory_sum[, parcel_count_ind] = apply(current_parcel_trajectory, 3, sum)
    parcel_counter_sum[, parcel_count_ind] = apply(parcel_counterfactual, 3, sum)
    initial_sum[parcel_count_ind] = sum(parcel_ecology)
  }
  
  parcel_sets_object$traj_rel_initial[[parcel_set_index]] = traj_rel_initial
  parcel_sets_object$initial_rel_counter[[parcel_set_index]] = initial_rel_counter
  parcel_sets_object$parcel_trajectory_sum[[parcel_set_index]] = parcel_trajectory_sum
  parcel_sets_object$initial_sum[[parcel_set_index]] = initial_sum
  
  parcel_sets_object$traj_rel_initial_set = cbind(parcel_sets_object$traj_rel_initial_set, rowSums(traj_rel_initial))
  parcel_sets_object$initial_rel_counter_set = cbind(parcel_sets_object$initial_rel_counter_set, rowSums(initial_rel_counter))
  parcel_sets_object$parcel_set_trajectories = cbind(parcel_sets_object$parcel_set_trajectories, rowSums(parcel_trajectory_sum))
  parcel_sets_object$parcel_set_counters = cbind(parcel_sets_object$parcel_set_counters, rowSums(parcel_counter_sum))
  parcel_sets_object$initial_sum_set = c(parcel_sets_object$initial_sum_set, sum(initial_sum))
...
find_parcel_counterfactual <- function(parcel_trajectory, parcel_ecology, current_dec_rate, global_params, yr){
  ...
  if (yr == 1){parcel_counterfactual[, , 1] = parcel_ecology
  } else {parcel_counterfactual[, , 1:(yr-1)] = parcel_trajectory[, , 1:(yr - 1)]
  }
...
plot_parcel_sums <- function(plot_type, assess_type, assess_object, parcel_set_num, colr){
  ...
  if (assess_type == 'set'){
    plot_1a = plot_object$parcel_set_trajectories[, parcel_set_num]
    plot_1b = plot_object$parcel_set_counters[, parcel_set_num]
  ...
  plot(plot_1a, ylim = c(mn, mx), type = 'l', xlab="time", ylab="parcel_sum", col = colr)
  lines(plot_1b, ylim = c(mn, mx), col = 'black', lty = 2)
  ...
  plot(plot_2a, ylim = c(mn, mx), type = 'l', xlab="time", col = colr)
  lines(plot_2b, ylim = c(mn, mx), col = 'black', lty = 2)
  lines(plot_2c, ylim = c(mn, mx))
...
plot_parcel_sets <- function(assess_object, assess_type, parcel_set_num){
  ...
  dim(sub_plots) = c(2, 3)
  layout(sub_plots)
  plot_parcel_sums(plot_type = 'developments', assess_type, assess_object, parcel_set_num, colr = 'red')
  plot_parcel_sums(plot_type = 'offsets', assess_type, assess_object, parcel_set_num, colr = 'blue')
  ...
  plot(plot_a, type = 'l', col = 'red', ylim = c(mn, mx))
  lines(plot_b, col = 'blue', ylim = c(mn, mx))
  lines((plot_a + plot_b), col = 'black', lty = 2)
...
plot_net_trajectories <- function(assess_object){
  graphics.off()
  sub_plots = 1:3
  dim(sub_plots) = c(1, 3)
  layout(sub_plots)
  ...
  two_plot(plot_a, plot_b, cols = c('black', 'black'), ltys = c(1, 2))
  lines((plot_a + plot_b), col = 'red')
  grid(nx = NULL, ny = NULL, col = "darkgray", lty = "dotted")
  
  two_plot(plot_c, plot_d, cols = c('black', 'black'), ltys = c(1, 2))
  lines((plot_c + plot_d), col = 'blue')
  grid(nx = NULL, ny = NULL, col = "darkgray", lty = "dotted")

  two_plot(plot_a + plot_b, plot_c + plot_d, cols = c('red', 'blue'), lty = c(1, 1))
  lines((plot_a + plot_b) + (plot_c + plot_d), col = 'black')
  grid(nx = NULL, ny = NULL, col = "darkgray", lty = "dotted")
...
two_plot <- function(plot_a, plot_b, cols, ltys){
  mx = max(c(plot_a, plot_b))
  mn = min(c(plot_a, plot_b))
  plot(plot_a, type = 'l', col = cols[1], ylim = c(mn, mx), lty = ltys[1])
  lines(plot_b, col = cols[2], ylim = c(mn, mx), lty = ltys[2])
}
...
extract_3D_parcel <- function(current_parcel, trajectories){
  ...
  dim(parcel_3D) = c(dim(current_parcel), dim(trajectories)[3])
  return(parcel_3D)
}
...
parcel_realisations <- function(traj_runs, parcel_ind, land_parcels, time_steps){
  ...
  for (realisation_ind in 1:realisation_num){
    parcel_3D = extract_3D_parcel(current_parcel, traj_runs[[realisation_ind]])
...
sum_development_vec <- function(dev_vec, parcel_num){
  sum_array = array(0, length(dev_vec))
  for (ind in 1:100){
    sum_array[ind] = sum(dev_vec[1:ind])
  }
  sum_array = 1 - sum_array/parcel_num
  return(sum_array)
}
