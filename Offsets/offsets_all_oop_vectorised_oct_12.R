initialise_parameters <- function(){
  ...
  params$update_every = 5  #how often the policy is implemented
...
initialise_outputs <- function(params, counterfactuals, parcels){
  ...
  outputs$parcel_sets$dev_counter = 1
  return(outputs)
...
select_parcel_to_develop <- function(params, ind_available, parcels, yr, current_ecology, final_ecology){
...
select_offset_parcel <- function(offset_pool, params, parcels, development_val, yr, current_ecology){      
...
write_parcel_sets <- function(parcel_sets, dev_counter, yr, region_ind, development_object, offset_object){
  parcel_set = list()
  parcel_set$yr = yr
  parcel_set$region = region_ind
  parcel_set$dev_parcel = development_object$parcel_index
...
collate_offset_parcel <- function(object){
  output_object = vector('list', 2)
  for (yr in 1:params$time_steps){
    output_object$parcel_list = c(output_object$parcel_list[[yr]], object$parcel_list)
    output_object$parcel_vals = c(output_object$parcel_vals[[yr]], object$parcel_vals)
  }
  object$parcel_list = parcel_list
  object$parcel_vals = parcel_vals
}
      ...
      for (region_ind in 1:parcels$region_num){
        ...
        if (develop_num > 0){
          for (dev_index in 1:develop_num){    #loop over the number of possible developments           
            ...
            development_object <- select_parcel_to_develop(params, index_object$ind_available[[region_ind]], parcels, yr, current_ecology, counterfactuals$final_ecology)
            ...
            offset_object <- select_offset_parcel(offset_pool, params, parcels, development_object$parcel_val_to_use, yr, current_ecology)                  
            ...
            outputs$parcel_sets <- write_parcel_sets(outputs$parcel_sets, dev_counter, yr, region_ind, development_object, offset_object)
