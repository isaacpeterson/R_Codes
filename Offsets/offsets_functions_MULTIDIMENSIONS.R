...
initialise_counterfactuals_vectorised <- function(global_params, region_params, land_parcels, initial_ecology, decline_rates_array){
  ...
  final_parcel_vals = find_current_parcel_sums(land_parcels, final_ecology, 1:length(land_parcels))
  ...
  counterfactuals_object$initial_decline_rates = decline_rates_array
...
build_decline_rates <- function(parcels, region_params){
  ...
  decline_rates$array = decline_rates_array
...
predict_parcel_vals <- function(predict_type, current_ecology, parcel_inds, land_parcels, decline_rates, offset_rate, min_eco_val, max_eco_val, time_step){
  ...
  current_decline_rates = decline_rates$array
...
update_decline_rates_array <- function(decline_rates_array, parcel_indexes, offset_rate){
  for (parcel_ind in 1:length(parcel_indexes)){
    current_parcel_ind = parcel_indexes[parcel_ind]
    decline_rates_array[current_parcel_ind] = offset_rate
  }
  return(decline_rates_array)
}
...
initialise_trajectories <- function(num_eco_dims){
  trajectories = vector('list', num_eco_dims)
  for (dm in 1:num_eco_dims){
    trajectories[[dm]] = array(0, c(global_params$ecology_size, global_params$ecology_size, global_params$time_steps))
  }
  return(trajectories)
}
  
calc_trajectories_by_parcel <- function(outputs, trajectories, global_params, region_params, current_ecology, decline_rates, parcels, index_object, counterfactuals_object, perform_offsets, record_parcel_sets){
  ...
  for (yr in 1:global_params$time_steps){      #main time loop    
      ...
      for (region_ind in 1:parcels$region_num){       
        ...
        if (current_develop_num > 0){
          ...
          for (dev_index in 1:current_develop_num){    #loop over the number of possible developments           
            ...
            decline_rates$array <- update_decline_rates_array(decline_rates$array, development_object$parcel_indexes, offset_rate = 0)
            ...
            if (perform_offsets == TRUE){
              ...
              decline_rates$array <- update_decline_rates_array(decline_rates$array, offset_object$parcel_indexes, offset_rate = region_params[[region_ind]]$offset_rate)
              if (index_object$break_flag == TRUE){
                break
              }               
            } else {
    ...
    current_ecology <- update_ecology_by_parcel(current_ecology, parcels$land_parcels, decline_rates$array, global_params$min_eco_val, global_params$max_eco_val, time_step = 1)
